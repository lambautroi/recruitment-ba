        ...
        const formattedCompanies = await Promise.all(
            companies.map(async (company) => {
                ...
                return {
                    ...
                    jobCount: jobCount, // ✅ SỐ JOB THỰC TẾ
...
router.get("/:id", async (req, res) => {
    try {
        ...
        const jobs = await Job.find({
            ...
            .populate("experience_id", "experience_level") // ✅ SỬA TỪ experience -> experience_id
            .populate("education_id", "education_level") // ✅ SỬA TỪ education -> education_id
            .populate("form_of_employment_id", "form_name") // ✅ SỬA TỪ form_of_employment -> form_of_employment_id
            .sort({ createdAt: -1 });

        const formattedEmployer = {
            _id: employer._id,
            name: employer.employer_name,
            logo: employer.employer_logo,
        ...
        const formattedJobs = jobs.map((job) => ({
            ...
            experience_name:
                job.experience_id?.experience_level || "Chưa xác định", // ✅ SỬA
            education_name:
                job.education_id?.education_level || "Chưa xác định", // ✅ SỬA
            form_name: job.form_of_employment_id?.form_name || "Chưa xác định", // ✅ SỬA
